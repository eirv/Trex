 ...
 * @date 2023/2/6 15:15
 */
public class TrexSpan {
    // Copy from
    // https://github.com/termux/termux-app/blob/master/terminal-emulator/src/main/java/com/termux/terminal/TerminalColorScheme.java#L14
    /**
    ...
    public static CharSequence from(String text) {
        char[] chars = text.toCharArray();
        int[] colorScheme = DEFAULT_COLORSCHEME;
        int colorSchemeLength = colorScheme.length;
        ...
        for (int i = 0; len > i; i++) {
            if (chars[i] == '\u001B' && chars[i + 1] == '[') {
                int endIndex = indexOf(chars, i + 2, Math.min(len, i + 14), 'm');
                if (endIndex == -1) continue;
                int[] values = splitInts(chars, i + 2, endIndex - 1, ';');
                if (values == null) continue;
                int valuesLen = values.length;

                int x = values[0];
                int y = valuesLen > 1 ? values[1] : -1;
                int z = valuesLen > 2 ? values[2] : -1;

                if (x == 0 && y == -1 && z == -1) {
                    ssb.append(new String(chars, lastStart, i - lastStart));
                    if (lastSpan != null) {
                        ssb.setSpan(
                                lastSpan,
                                spanStart,
                    ...
                    spanStart += i - lastStart;
                    lastSpan = null;
                    lastStart = endIndex;
                } else if (x == 38 && y == 5 && z != -1 && z < colorSchemeLength) {
                    int color = colorScheme[z];
                    if (lastStart == 0) {
                        ssb.append(new String(chars, 0, i));
                        lastSpan = new ForegroundColorSpan(color);
                        lastStart = endIndex;
                        spanStart = i;
                    } else {
                        ssb.append(new String(chars, lastStart, i - lastStart));
        ...
        ssb.append(new String(chars, lastStart, len - lastStart));
        ssb.setSpan(lastSpan, spanStart, ssb.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

        return ssb;
    }

    private static int indexOf(char[] chars, int off, int len, char ch) {
        while (off < len) {
            if (chars[off++] == ch) return off;
        }
        return -1;
    }

    private static int[] splitInts(char[] chars, int off, int len, char ch) {
        IntArray ints = new IntArray(3);
    ...
    private static int parseInt(char[] chars, int off, int len) {
        int result = 0;
        for (int i = 0, size = len - off - 1; size >= i; i++) {
            char ch = chars[off + i];
            if (ch < '0' || '9' < ch) throw new NumberFormatException();
            int value = ch - '0';
            result += value * powerOf10(size - i);
        }
        return result;
    }

    private static int powerOf10(int pow) {
