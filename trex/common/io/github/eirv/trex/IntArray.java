...
class IntArray {
     ...
     */
    private static int checkArgumentInRange(int value, int lower, int upper, String valueName) {
        if (value < lower) {
     ...
     */
    private static int checkArgumentNonnegative(final int value) {
        if (value < 0) {
            throw new IllegalArgumentException();
        }
     ...
     *     point) - 1)</i>. The insertion point is defined as the point at which the key would be
     *     inserted into the array: the index of the first element greater than the key, or {@link
     *     #size()} if all elements in the array are less than the specified key. Note that this
     *     guarantees that the return value will be >= 0 if and only if the key is found.
     */
    public int binarySearch(int value) {
        return binarySearch(mValues, mSize, value);
    }
            ...
            final int targetCap =
                    currentSize
                            + (currentSize < (MIN_CAPACITY_INCREMENT / 2)
                                    ...
                                    : currentSize >> 1);
            final int newCapacity = Math.max(targetCap, minCapacity);
