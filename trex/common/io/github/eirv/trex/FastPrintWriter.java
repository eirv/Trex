...
class FastPrintWriter extends PrintWriter implements TrexPrinter {
    ...
    private TrexOption mOption;
    private boolean mColorSchemeEnabled;

    /**
     * Constructs a new {@code PrintWriter} with {@code wr} as its target writer. By default, the
     * new print writer does not automatically flush its contents to the target writer when a
     * newline is encountered.
     *
     * <p>NOTE: Unlike PrintWriter, this version will still do buffering inside of FastPrintWriter
     * before sending data to the Writer. This means you must call flush() before retrieving any
    ...
    /**
     * Constructs a new {@code PrintWriter} with {@code wr} as its target writer. The parameter
     * {@code autoFlush} determines if the print writer automatically flushes its contents to the
     * target writer when a newline is encountered.
     *
     * @param wr the target writer.
    ...
    /**
     * Constructs a new {@code PrintWriter} with {@code wr} as its target writer and a custom buffer
     * size. The parameter {@code autoFlush} determines if the print writer automatically flushes
     * its contents to the target writer when a newline is encountered.
     *
     * @param wr the target writer.
     * @param autoFlush indicates whether to flush contents upon encountering a newline sequence.
     * @param bufferLen specifies the size of the FastPrintWriter's internal buffer; the default is
     *     8192.
     * @throws NullPointerException if {@code wr} is {@code null}.
     */
    public FastPrintWriter(Writer wr, boolean autoFlush, int bufferLen) {
        super(new DummyWriter(), autoFlush);
        Utils.requireNonNull(wr, "wr");
        mBufferLen = bufferLen;
        mText = new char[bufferLen];
        mWriter = wr;
        mAutoFlush = autoFlush;
        mSeparator = Utils.LINE_SEPARATOR;
    }
    ...
    /**
     * Ensures that all pending data is sent out to the target. It also flushes the target. If an
     * I/O error occurs, this writer's error state is set to {@code true}.
     */
    @Override
    public void flush() {
        try {
    ...
    /**
     * Prints a string to the target. The string is converted to an array of bytes using the
     * encoding chosen during the construction of this writer. The bytes are then written to the
     * target with {@code write(int)}.
     *
    ...
    /**
     * Prints the string representation of the char {@code c} followed by a newline. Flushes this
     ...
     *
     * <p>This writer's error flag is set to {@code true} if this writer is closed or an I/O error
     * occurs.
     *
    ...
    /**
     * Appends a subsequence of the character sequence {@code csq} to the target. This method works
     * the same way as {@code PrintWriter.print(csq.subsequence(start, end).toString())}. If {@code
     * csq} is {@code null}, then the specified subsequence of the string "null" will be written to
     * the target.
     *
     ...
     * @param end the index of the character following the last character of the subsequence
     *     appended to the target.
     * @return this writer.
     * @throws StringIndexOutOfBoundsException if {@code start > end}, {@code start < 0}, {@code end
     *     < 0} or either {@code start} or {@code end} are greater or equal than the length of
     *     {@code csq}.
     */
    @Override
    ...
    public void option(TrexOption option) {
        mOption = option;
        mColorSchemeEnabled = option.isColorSchemeEnabled();
    }

    @Override
    public void color(byte colorId) {
        if (mColorSchemeEnabled) {
            print(mOption.getColor(colorId));
        }
    }
